  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <h4 className="text-lg font-serif font-bold text-gray-900 dark:text-white whitespace-nowrap">⚡ Legacy → Modern</h4>
      ...
      {(comparison.legacyImage || comparison.modernImage) && (
        <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {/* Legacy Side */}
          <div className="relative rounded-xl overflow-hidden border border-gray-300 dark:border-white/10 flex flex-col h-64 bg-gray-100 dark:bg-gray-900/50">
      ...
      <div className="rounded-xl border border-gray-300 dark:border-white/10 overflow-hidden">
        {/* Table header */}
        <div className="grid grid-cols-[2fr_3fr_3fr] bg-gray-100 dark:bg-white/5 text-xs font-semibold tracking-widest uppercase">
          <div className="px-4 py-3 text-gray-600 dark:text-gray-500 border-r border-gray-300 dark:border-white/10">Aspect</div>
          <div className="px-4 py-3 text-red-600 dark:text-red-400/80 border-r border-gray-300 dark:border-white/10">✕ {legacyLabel}</div>
          <div className="px-4 py-3 text-emerald-600 dark:text-emerald-400/80">✓ {modernLabel}</div>
        </div>
        ...
        {comparison.items.map((item, i) => (
          <motion.div
            key={item.aspect}
            className={`grid grid-cols-[2fr_3fr_3fr] border-t border-gray-300 dark:border-white/[0.06] ${
          ...
          >
            <div className="px-4 py-3 border-r border-gray-300 dark:border-white/[0.06]">
              <span className="text-xs font-semibold text-gray-700 dark:text-gray-400">{item.aspect}</span>
              {item.improvement && (
            ...
            <div className="px-4 py-3 border-r border-gray-300 dark:border-white/[0.06]">
              <span className="text-xs text-red-700 dark:text-red-300/70 leading-relaxed">{item.legacy}</span>
            </div>
            <div className="px-4 py-3">
              <span className="text-xs text-emerald-700 dark:text-emerald-300/80 leading-relaxed">{item.modern}</span>
            </div>
          </motion.div>
        ))}
      </div>
...
const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  return (
    <div className="relative z-[1]">
      ...
      <motion.div
        className="border border-base-300 dark:border-base-300/20 rounded-lg mb-6 overflow-hidden transition-all duration-400 bg-white dark:bg-gray-900/40 hover:shadow-lg"
        whileHover={{ y: -5 }}
      ...
      >
        ...
        >
          <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-6 w-full">
            <div className="flex justify-between items-start w-full md:w-auto">
              ...
              <motion.div
                ...
                className="bg-gray-100 dark:bg-gray-800 rounded-full p-2 flex-shrink-0 ml-4 md:hidden mt-2 text-gray-800 dark:text-gray-200"
              >
            ...
            <div className="flex-1 mt-2 md:mt-0 max-w-full">
              <h3 className="text-lg md:text-xl font-bold pr-8 md:pr-0 text-gray-900 dark:text-white">{title}</h3>
              <p className="text-sm md:text-base mt-1 md:mt-2 text-gray-600 dark:text-gray-400 line-clamp-2 md:line-clamp-none">{description}</p>
              <div className="mt-3 flex flex-wrap gap-2">
                {technologies.split(",").slice(0, 3).map((tech, index) => (
                  <span key={index} className="px-2 py-1 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-md text-[10px] md:text-xs border border-gray-200 dark:border-transparent">
                    {tech.trim()}
                  </span>
                ))}
                {technologies.split(",").length > 3 && (
                  <span className="px-2 py-1 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-md text-[10px] md:text-xs border border-gray-200 dark:border-transparent">
          ...
          <motion.div
            animate={{ rotate: isOpen ? 180 : 0 }}
            transition={{ duration: 0.5 }}
            className="hidden md:block bg-gray-100 dark:bg-gray-800 rounded-full p-2 flex-shrink-0 ml-4 text-gray-800 dark:text-gray-200"
          >
              ...
              <div className="p-6 pt-0">
                <div className="grid md:grid-cols-3 gap-8">
                  ...
                  <div className="md:col-span-2">
                    <motion.h4 className="text-lg font-semibold mb-4 text-gray-900 dark:text-white" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
                      Project Overview
                    </motion.h4>
                    <motion.p className="mb-6 text-gray-700 dark:text-gray-300 leading-relaxed" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                      {description}
                    </motion.p>
                  ...
                  <div className="space-y-6 self-start sticky top-8">
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {technologies.split(",").map((tech, index) => (
                          <span key={index} className="px-3 py-1.5 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-md text-sm border border-gray-200 dark:border-transparent">
                            {tech.trim()}
                          </span>
                        ))}
                      </div>
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.35 }}>
                      <h4 className="text-lg font-semibold mb-2 text-gray-900 dark:text-white">Duration</h4>
                      <p className="text-gray-700 dark:text-gray-300">{duration}</p>
                    </motion.div>

                    {allImages.length > 1 && (
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.38 }}>
                        <h4 className="text-lg font-semibold mb-2 text-gray-900 dark:text-white">Gallery</h4>
                        <button
                    ...
                    {normalizedLinks.length > 0 && (
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}>
                        <h4 className="text-lg font-semibold mb-3 text-gray-900 dark:text-white">Links</h4>
                        <div className="flex flex-wrap gap-3">
                          {normalizedLinks.map((link, index) => (
                            <motion.a
                              ...
                              className="btn btn-sm btn-outline text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white flex items-center gap-2 transition-colors"
                  ...
                  <div className="grid md:grid-cols-2 gap-8">
                    ...
                    {role && (
                      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }}>
                        <h4 className="text-lg font-semibold mb-2 text-gray-900 dark:text-white">My Role</h4>
                        <p className="text-gray-700 dark:text-gray-300 leading-relaxed">{role}</p>
                      </motion.div>
                    )}
